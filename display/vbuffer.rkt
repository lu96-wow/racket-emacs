...
;;
;;   ── Mutations (in-place vector-set! for performance) ──
;;     vbuffer-clear! vbuffer-put-char! vbuffer-put-string!
;;     vbuffer-fill! vbuffer-blit!
;;
;; ============================================================================
;; Dependencies: none (zero dependencies on kernel or platform)
;; ============================================================================
...
(struct cell
  (ch            ; char? — character to display
   [attrs #:mutable]      ; (or/c #f symbol? (listof symbol?)) — bold, etc.
...
(define (make-vbuffer rows cols)
  (unless (and (exact-positive-integer? rows) (exact-positive-integer? cols))
    (raise-argument-error 'make-vbuffer "positive integers" (list rows cols)))
...
(define (vbuffer-clear! vb)
  (define len (vector-length (vbuffer-cells vb)))
  (for ([i (in-range len)])
...
(define (vbuffer-blit! dst dst-top dst-left src)
  ...
  ;; Handles clipping at dst boundaries.
  (unless (vbuffer? dst)
...
(define (vbuffer-row->string vb row)
  (unless (and (>= row 0) (< row (vbuffer-rows vb)))
    (raise-argument-error 'vbuffer-row->string
                          (format "row in [0, ~a)" (vbuffer-rows vb)) row))
  (define cols (vbuffer-cols vb))
  (list->string
