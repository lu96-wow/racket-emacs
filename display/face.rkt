...
(for-each (λ (pair) (unless (face-defined? (car pair)) (defface (car pair) (cdr pair))))
          (list (cons font-lock-string-face (make-face-attrs 'foreground 2))
                ...
                ;; Rainbow parens — 12 depth levels, ~2.5× brighter
                ;; Red (face-1) moved to face-12, all others shifted up by 1
                (cons font-lock-paren-face-1  (make-face-attrs 'background (list 38 83 53)))
                (cons font-lock-paren-face-2  (make-face-attrs 'background (list 38 60 95)))
                (cons font-lock-paren-face-3  (make-face-attrs 'background (list 95 38 83)))
                (cons font-lock-paren-face-4  (make-face-attrs 'background (list 95 80 38)))
                (cons font-lock-paren-face-5  (make-face-attrs 'background (list 38 95 88)))
                (cons font-lock-paren-face-6  (make-face-attrs 'background (list 75 38 95)))
                (cons font-lock-paren-face-7  (make-face-attrs 'background (list 95 68 30)))
                (cons font-lock-paren-face-8  (make-face-attrs 'background (list 30 75 38)))
                (cons font-lock-paren-face-9  (make-face-attrs 'background (list 38 68 95)))
                (cons font-lock-paren-face-10 (make-face-attrs 'background (list 95 38 60)))
                (cons font-lock-paren-face-11 (make-face-attrs 'background (list 83 95 38)))
                (cons font-lock-paren-face-12 (make-face-attrs 'background (list 95 53 38)))))
