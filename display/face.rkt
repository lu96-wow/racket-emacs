...
(for-each (λ (pair) (unless (face-defined? (car pair)) (defface (car pair) (cdr pair))))
          (list (cons font-lock-string-face (make-face-attrs 'foreground 2))
                ...
                ;; Rainbow parens — 12 depth levels, HSV golden-angle palette
                ;; RGBA blended on dark (#000) background at 15% opacity → effective RGB
                (cons font-lock-paren-face-1  (make-face-attrs 'background (list 38 21 15)))
                (cons font-lock-paren-face-2  (make-face-attrs 'background (list 15 33 21)))
                (cons font-lock-paren-face-3  (make-face-attrs 'background (list 15 24 38)))
                (cons font-lock-paren-face-4  (make-face-attrs 'background (list 38 15 33)))
                (cons font-lock-paren-face-5  (make-face-attrs 'background (list 38 32 15)))
                (cons font-lock-paren-face-6  (make-face-attrs 'background (list 15 38 35)))
                (cons font-lock-paren-face-7  (make-face-attrs 'background (list 30 15 38)))
                (cons font-lock-paren-face-8  (make-face-attrs 'background (list 38 27 12)))
                (cons font-lock-paren-face-9  (make-face-attrs 'background (list 12 30 15)))
                (cons font-lock-paren-face-10 (make-face-attrs 'background (list 15 27 38)))
                (cons font-lock-paren-face-11 (make-face-attrs 'background (list 38 15 24)))
                (cons font-lock-paren-face-12 (make-face-attrs 'background (list 33 38 15)))))
