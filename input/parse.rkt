...
(module+ test
  ...
  (test-case "CSI arrow key decode"
    ;; Simulate CSI sequence bytes for UP: ESC [ A = 27 91 65
    (define seq (bytes 27 91 65))
    (define-values (ps final) (parse-csi-params seq))
    (check-equal? final 65)
    (define ke (csi-to-key ps final))
    (check-true (key-sym? ke))
    (check-equal? (key-sym-name ke) 'up))

  (test-case "CSI delete key decode"
    ;; ESC [ 3 ~ = 27 91 51 126
    (define seq (bytes 27 91 51 126))
    (define-values (ps final) (parse-csi-params seq))
    (check-equal? (car ps) 3)
    (define ke (csi-to-key ps final))
    (check-true (key-sym? ke))
    (check-equal? (key-sym-name ke) 'delete))

  (test-case "SGR mouse decode"
    ;; ESC [ < 0 ; 10 ; 20 M  (left press at col 10, row 20)
    (define params '(0 10 20))
    (call-with-values (λ () (decode-sgr-mouse params MOUSE-EVENT))
      (λ (type button x y action mods)
        (check-eq? type 'mouse)
        (check-eq? button 'left)
        (check-equal? x 10)
        (check-equal? y 20)
        (check-eq? action 'press))))

  (test-case "SGR mouse scroll"
    ;; ESC [ < 64 ; 5 ; 10 M  (scroll up at col 5, row 10)
    (define params '(64 5 10))
    (call-with-values (λ () (decode-sgr-mouse params MOUSE-EVENT))
      (λ (type button x y action mods)
        (check-eq? type 'mouse)
        (check-eq? button 'wheel-up)
        (check-equal? x 5)
        (check-equal? y 10)
        (check-eq? action 'scroll))))
