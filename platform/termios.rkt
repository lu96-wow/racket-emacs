...
;;
;; FFI-based. No editor dependencies.
...
(define (copy-termios src)
  ...
  (bytes-copy! dst 0 src 0 TERMIOS-SIZE)
  dst)
...
(define (exit-raw-mode!)
  ...
  ;; Even if raw mode wasn't entered, ensure cursor is visible
  (display "\e[?25h")
  (display "\e[0m")
  (flush-output))
...
(define (screen-cleanup!)
  ;; Safe to call even if init failed — restore terminal state
