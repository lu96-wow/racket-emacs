...
;; platform/event.rkt — Terminal input: raw bytes → key-event / mouse-event
;;
;; Reads from stdin, parses: ASCII, CSI escape sequences, UTF-8,
;; 8-bit Meta, SGR mouse reports.  Produces key-event or mouse-event.
;;
;; This is the only module in platform/ that reads from stdin.
;; Dependencies: kernel/key-event (pure types)
...
(provide
 ...
 ;; constants (used by other modules for detection)
 TAB LF CR ESC SPACE)
...
(define (read-key-event!)
  ...
  (cond
    ...
    [(<= 128 b 255)
     (define lo (- b 128))
     (cond [(= lo (char->integer #\[)) (parse-escape)]
           [(= lo (char->integer #\O)) (parse-escape)]
           [(<= 32 lo 126) (key-event (integer->char lo) #f #t #f #f)]
           [else (key-event (integer->char lo) #t #t #f #f)])]
...
(define (read-csi-seq b2 [b3 #f])
  (let loop ([acc (if b3 (list ESC b2 b3) (list ESC b2))])
    ...
    (cond [(not b) (list->bytes acc)]
          [(<= CSI-FINAL-START b CSI-FINAL-END)
           (list->bytes (append acc (list b)))]
...
(define (decode-sgr-mouse seq)
  ...
  (define-values (y  _off) (parse-mouse-field seq (+ 3 _1 1 off2 1) final-b))
  (define scroll? (>= cb 64))
  ...
  (define btn (case raw-btn
                ...
                [else 'unknown]))
  (define action (cond [scroll? (if (= (bitwise-and cb 1) 0)
