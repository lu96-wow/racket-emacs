...
(module+ test
  ...
  (test-case "scan-word-forward basic"
    (define gb (make-gap-buffer "hello world foo"))
    (define p (scan-word-forward gb 0 (gap-length gb) st))
    (check-equal? (gap-substring gb 0 p) "hello ")
    ;; Next word
    (define p2 (scan-word-forward gb p (gap-length gb) st))
    (check-equal? (gap-substring gb p p2) "world "))

  (test-case "scan-word-forward at buffer end"
    (define gb (make-gap-buffer "end"))
    (define p (scan-word-forward gb 0 (gap-length gb) st))
    (check-equal? p (gap-length gb)))

  (test-case "scan-word-backward basic"
    (define gb (make-gap-buffer "hello world foo"))
    (define p (scan-word-backward gb (gap-length gb) st))
    ;; First backward-word from end: should land at start of "foo"
    (check-equal? (gap-substring gb p (gap-length gb)) "foo")
    ;; Second backward-word: should land at start of "world"
    (define p2 (scan-word-backward gb p st))
    (check-equal? (gap-substring gb p2 p) "world "))

  (test-case "scan-word-backward at buffer start"
    (define gb (make-gap-buffer "hello"))
    (define p (scan-word-backward gb (gap-length gb) st))
    (check-equal? p 0))

  (test-case "scan-list-forward simple"
    (define gb (make-gap-buffer "(a b c) after"))
    ;; scan-list-forward returns position just past the closing delimiter.
    ;; It skips over the closing paren, so the returned position is after ')'.
    (define p (scan-list-forward gb 0 (gap-length gb) st #\())
    (check-equal? (gap-substring gb 0 p) "(a b c)"))

  (test-case "scan-list-forward nested"
    (define gb (make-gap-buffer "(a (b) c) after"))
    (define p (scan-list-forward gb 0 (gap-length gb) st #\())
    (check-equal? (gap-substring gb 0 p) "(a (b) c)"))

  (test-case "scan-sexp-forward string"
    (define gb (make-gap-buffer "\"hello world\" after"))
    (define p (scan-sexp-forward gb 0 (gap-length gb) st))
    (check-equal? (gap-substring gb 0 p) "\"hello world\""))

  (test-case "scan-sexp-forward expression prefix"
    (define gb (make-gap-buffer "'(a b c) after"))
    (define p (scan-sexp-forward gb 0 (gap-length gb) st))
    (check-equal? (gap-substring gb 0 p) "'(a b c)"))

  (test-case "skip-char-class-forward"
    (define gb (make-gap-buffer "abc   def"))
    (define p (skip-char-class-forward gb 0 (gap-length gb) st 'word))
    (check-equal? (gap-substring gb 0 p) "abc")
    (define p2 (skip-char-class-forward gb p (gap-length gb) st 'whitespace))
    (check-equal? (gap-substring gb p p2) "   ")))
