...
(define global-registry (buffer-registry (make-hash) (make-hash) '()))

(define (register-buffer! buf)
  (define name (buffer-name buf)) (define ci (string-downcase name))
  (hash-set! (buffer-registry-by-name global-registry) name buf)
  (hash-set! (buffer-registry-by-name-ci global-registry) ci buf))

(define (unregister-buffer! buf)
  (define name (buffer-name buf))
  (hash-remove! (buffer-registry-by-name global-registry) name)
  (hash-remove! (buffer-registry-by-name-ci global-registry) (string-downcase name)))

(define (get-buffer-create name #:inhibit-hooks? [inhibit? #f])
  (define ci (string-downcase name))
  (or (hash-ref (buffer-registry-by-name-ci global-registry) ci #f)
      (let ([buf (make-buffer name)]) (register-buffer! buf) buf)))

(define (get-buffer name)
  (define ci (string-downcase name))
  (hash-ref (buffer-registry-by-name-ci global-registry) ci (λ () #f)))
...
(define (kill-buffer #:buf [b (current-buffer)])
  ...
  (for ([f (in-list (buffer-registry-on-kill global-registry))]) (f b))
...
(define (buffer-live-p b) (hash-has-key? (buffer-registry-by-name global-registry) (buffer-name b)))
(define (switch-to-buffer buf-or-name) (define buf (get-buffer-create buf-or-name)) (set-buffer buf) buf)
(define (buffer-list) (hash-values (buffer-registry-by-name global-registry)))
...
(define (rename-buffer newname)
  ...
  (hash-remove! (buffer-registry-by-name global-registry) oldname)
  (hash-remove! (buffer-registry-by-name-ci global-registry) (string-downcase oldname))
